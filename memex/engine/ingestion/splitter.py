...
def chunk_file(file_path_or_url: str, include_doc: bool = False, defer_ocr: bool = False) -> dict[str, Any]:
    ...
    When ``defer_ocr=True`` and MarkItDown output is poor quality, returns
    ``{"chunks": [], "markdown": "", "needs_ocr": True}`` so the caller can
    run OCR in a separate concurrency lane.
    """
    ...
    if config.CONVERTER_ENGINE == "marker":
        ...
        parse_result = parse_file(file_path_or_url, defer_ocr=defer_ocr)
        if parse_result.status == "needs_ocr":
            return {"chunks": [], "markdown": "", "needs_ocr": True}
